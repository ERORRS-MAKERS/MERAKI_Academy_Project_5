...
export default function IconBoxStyle12() {
  ...
  // Handles updating of an appointment
  const updateHandler = async (appointment_id) => {
    setLoading(true);
    setError(null);
  ...
  // Opens the update modal and sets the current appointment ID
  const openUpdateModal = (appointment_id) => {
    setCurrentAppointmentId(appointment_id);
    setShowModal(true);
  };

  // Renders the update modal
  const updateModal = () => (
    <div
      className="modal"
      style={{
            ...
            <div style={{ alignSelf: 'end' }}>
              <button
                ...
                style={{
                  ...
                  outline: '0',
                  color: '#FFF',
                  backgroundColor: '#2196F3', // Ensure there's a background color
                }}
                onClick={() => openUpdateModal(item.id)}
              >
                <svg
