...
import React,{useState} from "react"
 ...
 const MedicalReportsForm = ()=>{
    ...
    axios.get(`http://localhost:5000/reports/${national_id}`).then((result)=>{
...
<div className="col-lg-6">
  ...
  <input type="text" className="cs_form_field" placeholder="Patient National ID" onChange={(e)=>{
 
   if (e.target.value.length===6||e.target.value.length===9){
      saveData(e.target.value)
    }
  }}/>
 {userData && <span>Name :{userData.first_name +' ' + userData.last_name}</span>}
